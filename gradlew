...
if [ "$cygwin" = "true" -o "$msys" = "true" ] ; then
    ...
    # if the argument is an absolute path and its name ends with a # then prepend window path separator
    case $1 in
        -D*=-Djava.library.path=* )
            addl=`cygpath --windows "$2"`
            addl=`expr "$addl" : '\(.*\)/'`
            LIB_JARS="$addl"
            ;;
    esac
...
save () {
    for var in "$@"
    do
        printf %s\\n "$var" | sed -e 's/$/\\/' -e '$s/\\$//' >> "$CYGWIN_WRAPPERDIR/argfile"
    done
}
